...
const CaptainBlackCat = ({ className = "" }: { className?: string }) => (
  <div className={`relative w-20 h-20 flex-shrink-0 drop-shadow-2xl ${className}`}>
    <svg viewBox="0 0 100 100" className="w-full h-full">
      ...
      {/* Asalato */}
      <g className="animate-asalato origin-center" transform="translate(68, 65)">
        <path d="M0 0 Q4 -8 8 -16" stroke="#fbbf24" fill="none" strokeWidth="1.5" />
        <circle cx="0" cy="0" r="4" fill="#78350f" stroke="#451a03" strokeWidth="0.5" />
        <circle cx="8" cy="-16" r="4" fill="#78350f" stroke="#451a03" strokeWidth="0.5" />
      </g>
    </svg>
  </div>
);
...
const AssistantOrangeCat = ({ className = "" }: { className?: string }) => (
  <div className={`relative w-20 h-20 flex-shrink-0 drop-shadow-2xl ${className}`}>
    <svg viewBox="0 0 100 100" className="w-full h-full">
      ...
      {/* Vintage Fixed Focus Folding Opera Glass */}
      <g className="animate-peer origin-[40px_50px]" transform="translate(35, 45)">
        {/* Handle */}
        <rect x="-2" y="10" width="2" height="25" fill="#92400e" rx="1" />
        {/* Bridge / Folding Frame */}
  ...
  <div className={`relative w-64 h-64 flex items-center justify-center ${className}`}>
    ...
    <svg viewBox="0 0 200 200" className="w-full h-full animate-ship">
      ...
      <g className="text-blue-400">
        ...
        <text x="185" y="110" fontSize="18" className="animate-float" style={{ animationDelay: '2.4s' }}>♩</text>
      </g>
    </svg>
  </div>
);
  ...
  <div className="flex flex-col items-center justify-center min-h-[70vh] relative overflow-hidden">
    <div className="relative z-10 text-center space-y-12">
      ...
      <div className="space-y-4">
        ...
        <p className="text-blue-700 font-bold uppercase tracking-[0.5em] text-xs">Captains Andrea & Rita, provisions located!</p>
      </div>
    </div>
    <div className="absolute bottom-0 left-0 w-[200%] h-48 opacity-30 pointer-events-none">
      <svg className="animate-wave w-full h-full" viewBox="0 0 1000 100" preserveAspectRatio="none">
        <path d="M0 50 Q125 0 250 50 T500 50 T750 50 T1000 50 V100 H0 Z" fill="#1e40af" />
      </svg>
    </div>
  </div>
);
...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError('The galley search was interrupted by a squall. Please try again.');
      setStep('selection');
    }
  };
  ...
  return (
    <div className="pb-24">
      <header className="bg-blue-900 border-b-8 border-blue-950 sticky top-0 z-50 h-28 shadow-2xl flex items-center px-6">
        {/* Cats directly in the Upper Left Hand Corner */}
        <div 
          className="flex items-center gap-2 cursor-pointer group select-none" 
          onClick={() => setStep('selection')}
          aria-label="Return to Port"
        >
          <div className="flex -space-x-8 group-hover:space-x-1 transition-all duration-700 animate-cat-header">
            <CaptainBlackCat className="scale-[0.7] sm:scale-[0.8] origin-bottom-right" />
            <AssistantOrangeCat className="scale-[0.7] sm:scale-[0.8] origin-bottom-left" />
          </div>
      ...
      <main className="max-w-7xl mx-auto px-8 pt-16">
        {step === 'selection' && (
          <div className="animate-in fade-in slide-in-from-bottom-12 duration-1000">
            ...
            {error && (
              <div className="max-w-xl mx-auto mb-16 p-8 bg-red-50 border-l-[12px] border-red-600 text-red-950 rounded-3xl flex items-center gap-6 animate-bounce shadow-2xl">
            ...
            {activeTab === 'weekly' ? (
              ...
              <div className="max-w-5xl mx-auto pb-48 space-y-16">
                ...
                {plan.weekendPrep.map((group, i) => (
                  <div key={i} className="bg-white p-12 rounded-[55px] border-8 border-blue-50 shadow-3xl hover:border-blue-400 transition-all duration-500 group">
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-10">
                      <h4 className="text-3xl font-black text-blue-950 flex items-center gap-6">
                        <span className="w-14 h-14 bg-blue-800 text-white rounded-2xl flex items-center justify-center text-xl shadow-lg group-hover:rotate-12 transition-transform">{i+1}</span>
                        {group.title}
                      </h4>
                      <div className="bg-sky-50 text-sky-700 font-black px-6 py-2 rounded-2xl border-2 border-sky-100 flex items-center gap-2 shadow-sm">
                        ...
                        {group.prepTimeMinutes} MINS
                      </div>
                    </div>
                    <ul className="grid md:grid-cols-2 gap-8">
                      {group.tasks.map((task, j) => (
                        <li key={j} className="flex gap-5 items-start text-slate-700 font-black text-lg">
                          <div className="w-8 h-8 rounded-xl border-4 border-blue-100 mt-1 flex-shrink-0 group-hover:border-blue-500 transition-colors" />
                          {task}
                        </li>
                      ))}
                    </ul>
                ...
                <div className="flex justify-center pt-20">
                  <button onClick={() => setStep('selection')} className="bg-white border-8 border-blue-100 px-16 py-6 rounded-full text-blue-950 font-black text-2xl hover:bg-blue-50 shadow-3xl transition-all hover:-translate-y-2 active:scale-95">RETURN TO PORT ⚓</button>
                </div>
              </div>
            )}
          </div>
